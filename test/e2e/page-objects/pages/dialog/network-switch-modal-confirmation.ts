...
class NetworkSwitchModalConfirmation {
  ...
  private readonly addNetworkMessage = { text: 'Want to add this network?', tag: 'h3' };
  private readonly seeDetailsButton = { tag: 'a', text: 'See details' };

  async check_pageIsLoaded(): Promise<void> {
    try {
      await this.driver.waitForMultipleSelectors([this.addNetworkMessage, this.submitButton]);
    } catch (e) {
      console.log('Timeout while waiting for add network confirmation modal to be loaded', e);
      throw e;
    }
  ...
  async check_networkInformationIsDisplayed({
    ...
    blockExplorerURL: string;
     ...
     console.log('Check network information is correctly displayed on network switch modal');
     await this.driver.waitForMultipleSelectors([{ text: networkURL, tag: 'dd' }, { text: currencySymbol, tag:'dd' }]);
